...
function removeFirstOccurrences(str, value) {
  const fistLetterOccurrences = str.indexOf(value);

  if (fistLetterOccurrences === -1) {
    return str;
  }

  const lastLetterOccurrences = fistLetterOccurrences + value.length;

  return str
    .slice(0, fistLetterOccurrences)
    .concat(str.slice(lastLetterOccurrences));
}
...
function removeLastOccurrences(str, value) {
  const fistLetterOccurrences = str.lastIndexOf(value);
...
function countVowels(str) {
  return str.toLowerCase().replace(/[^a|e|i|o|u|y]/gi, '').length;
}
...
function encodeToRot13(str) {
  ...
  const alphabetPOT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  return str.replace(/[A-Za-z]/gi, (char) => {
    return alphabetPOT13[alphabet.indexOf(char)];
  });
}
